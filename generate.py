...
# Фильтр тестовых uid — раньше был НЕГАТИВНЫМ (исключить чисто числовые +
# один known-test id), из-за чего в статистику попадала целая волна тестовых
# аккаунтов с разработки (dev-test-cat1..40, qa-*-j23, offline-test-user,
# claude-audit-*, опечатки типа 153t/153y) — заказчик заметил всплеск «тестовых
# пользователей» в июле 2026 (как раз пик тестирования в этой сессии) на
# графике «Вовлечённость по месяцам». Переведено на ПОЗИТИВНЫЙ фильтр: реальный
# uid с pharmskills — это base64-подобный токен (пример: sbCCwTOycrqzBZXz8cpz
# zDRsEfioJvFXRUhUc2Z4q0g=), длиной от 20 символов, без дефисов/подчёркиваний
# (все наши dev-test-*/qa-* именно с дефисами — под паттерн не попадают).
# Пробел в алфавите — НЕ опечатка: часть реальных uid содержит буквальный
# пробел вместо '+' (сервер отдающий uid не percent-encode'ит '+' в query,
# браузер/сервер декодирует его как пробел до сохранения в БД) — без пробела
# в алфавите ~15 реальных пользователей ошибочно считались бы тестовыми.
# НЕ требуем длину кратную 4 (строгий признак валидного base64) — проверено
# на реальных данных: часть настоящих токенов приходит с одним потерянным
# символом (та же history с '+'/пробелом), кратность 4 отсеивала ~13 реальных
# uid как псевдо-тестовые. Даёт 672 реальных / 37 тестовых на проверке по
# полной таблице pharmacy_users (было бы неотличимо при негативном фильтре).
#
# ДОПОЛНИТЕЛЬНО (заказчик, после проверки июля 2026 — месяца пиковой
# разработки): часть тестировщиков заходила НЕ под dev-test-cat*/qa-*-j23, а
# под СВОИМИ настоящими pharmskills-аккаунтами (реальный base64 uid — по
# формату неотличим от обычного клиента, заказчик сам это подтвердил на
# примерах «Лика»/«аптекарь»). Полностью выявить такие невозможно (формат
# идентичен обычному клиенту), но per direct instruction — минимум отсекаем
# КОРОТКИЕ uid: 604 из 672 реальных прошедших фильтр — 44 символа (32-байтный
# токен, основной/массовый тип), а 47 — всего 24 символа (16-байтный, другой,
# более редкий тип генерации) — именно в этой короткой группе оказались явно
# подозрительные записи июля («Balumba»/«Risus»/«эля»/«ната», короткие uid
# вида 'g7zVaDY8lZCcN6JwurM4tg=='). len>=40 сохраняет доминирующий 43/44-
# символьный тип (и более редкий 63/64), отсекает 24/23-символьный.
REAL = r"pu.uid ~ '^[A-Za-z0-9+/ ]{20,}={0,2}$' AND LENGTH(pu.uid) >= 40"
